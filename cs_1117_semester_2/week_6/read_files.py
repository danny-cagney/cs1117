...
You can associate this file object with a variable when you open a file using the with and as keywords. 
...
Pythons json module allows reading such data format and parsing it to a dictionary. 
...
"""
Writing to an opened file with the 'w' flag overwrites all previous content in the file. 
To avoid this, we can append to a file instead. Use the 'a' flag as the second argument to open(). 
...
"""
By default, a file when opened with open() is only for reading. 
A second argument 'r' is passed to it by default. 
To write to a file, first open the file with write permission via the 'w' argument. 
Then use the .write() method to write to the file. 
...
each representing an individual line in the file. 
...
#Python Read Method
"""
After a file is opened with open() returning a file object, 
call the .read() method of the file object to return the entire file content as a Python string. 
